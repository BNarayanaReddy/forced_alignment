...
def generate_dictionary(corpus_dir, g2p_model, output_dict_path, base_dictionary):
    command = [
        ...
        "--dictionary_path", base_dictionary
    ]
...
def align_with_trained_model(corpus_dir, trained_dict_path, acoustic_model, output_alignment_path):
    command = [
        'mfa', 'align',
        corpus_dir,
        trained_dict_path,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a dictionary and align a corpus using MFA.")
    parser.add_argument("txt_dir", type=str, help="Path to the text only corpus directory.")
    parser.add_argument("corpus_dir", type=str, help="Path to the corpus directory.")
    parser.add_argument("base_dictionary", type=str, help="Path to the base dictionary (e.g., english_us_arpa).")
    parser.add_argument("g2p_model", type=str, help="Name of the G2P model to use (e.g., english_g2p).")
    parser.add_argument("acoustic_model", type=str, help="Name of the acoustic model to use for alignment (e.g., english_us_arpa).")
    ...
    generate_dictionary(args.txt_dir, args.g2p_model, generated_dict_path, base_dictionary=args.base_dictionary)

    # Step 2: Combine the base dictionary with the newly generated one
    print(f"Combining {args.base_dictionary} and {generated_dict_path} into {combined_dict_path}")
    combine_dictionaries(args.base_dictionary, generated_dict_path, combined_dict_path)

    # Step 3: Train a new dictionary model
    train_dictionary(args.corpus_dir, combined_dict_path, args.g2p_model, trained_dict_output_path)

    # Step 4: Align using the newly trained dictionary
    final_trained_dict = os.path.join(trained_dict_output_path, os.path.basename(combined_dict_path))
    align_with_trained_model(args.corpus_dir, final_trained_dict, args.acoustic_model, final_alignment_path)
